        ...
        <div className="grid grid-cols-1 md:lg:xl:grid-cols-3 group bg-white shadow-xl shadow-neutral-100 border ">
          {docs.map((element) => {
            ...
            if (element.doc_status === "Verified") {
              docStatusElement = <img className="pendingImage" src={ verified } alt="Pending"></img>;
              return (
                <div
                  key={element.doc_name}
                  className="p-10 flex flex-col items-center text-center group md:lg:xl:border-r md:lg:xl:border-b cursor-not-allowed"
                >
                  <span className="p-5 rounded-full bg-lightBlue text-white shadow-lg shadow-red-200">
                    <img src={element.doc_logo_url} alt={element.doc_name}></img>
                  </span>
            ...
            if (element.doc_status === "Pending") {
              docStatusElement = <img className="pendingImage" src={pending} alt="Pending"></img>;
              return (
                <div
                  key={element.doc_name}
                  className="p-10 flex flex-col items-center text-center group md:lg:xl:border-r md:lg:xl:border-b cursor-not-allowed"
                >
                  <span className="p-5 rounded-full bg-lightBlue text-white shadow-lg shadow-red-200">
                    <img className="" src={element.doc_logo_url} alt={element.doc_name}></img>
                  </span>
                  <p className="text-xl font-medium text-slate-700 mt-3">
                    {element.doc_name}
                  </p>
            ...
            if (element.doc_status === "Not Verified") {
              docStatusElement = <img className="pendingImage" src={verifyNow} alt="Pending"></img>;
              return (
                <a
                  key={element.doc_name}
                  href={element.doc_v}
